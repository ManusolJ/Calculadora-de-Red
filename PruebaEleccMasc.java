...
public class PruebaEleccMasc {
    public static void main(String[] args) {
        ...
        public static String Mascara(String opc){
            Scanner sc = new Scanner(System.in);
            String mascara = "0";
            switch (Integer.parseInt(opc)) {
                case 1:
                    System.out.println("Notación CDIR elegida.");
                    while (!mascara.matches("^/[0-3]?[0-9]") || Integer.parseInt(mascara.substring(1, mascara.length())) > 32 || Integer.parseInt(mascara.substring(1, mascara.length())) <= 0) {
                        try {
                            System.out.print("Escribe la máscara con notación CDIR: ");
                            mascara = sc.next();
                            if (!mascara.matches("^/[0-3]?[0-9]") || Integer.parseInt(mascara.substring(1, mascara.length())) > 32 || Integer.parseInt(mascara.substring(1, mascara.length())) <= 0)
