    ...
    public static void main(String [] args){
        ...
        for(int i = 0;i < ip.length();i++){
            ...
            if(c < 46 || c > 57 || c == 47 || !ip.contains(".") || ip.length() > 16){
                System.out.println("Error! Debes introducir una IP válida.");
                ip = sc.next();
                i = 0;
            }
        }//Se valida la IP introducida.

        String [] ipElementos = ip.split("\\.");    //Se divide los números en el punto,introduciéndolos en un array con 4 elementos.
        String resultado = "";
        for(String octavo : ipElementos){   //Se asigna cada elemento del array a un String,cada vez que se itera el bucle.
            ...
            String binario = Integer.toBinaryString(oct) + "."; //Este int se transforma en binario mediante un función.
            while(binario.length() != 9){   //Como los ceros a la izquierda se ignoran,se añade esta parte para que hayan exactamente 8 dígitos en el octeto.
